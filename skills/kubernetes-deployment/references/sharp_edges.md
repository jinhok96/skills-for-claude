...
## Crashloop Backoff Spiral

### **Id**
crashloop-backoff-spiral
### **Summary**
CrashLoopBackOff looks like app crash but it's often environment/config issue
### **Severity**
critical
### **Situation**
Pod stuck in CrashLoopBackOff, logs show immediate exit or cryptic errors
### **Why**
...
### **Detection Pattern**
CrashLoopBackOff|restartCount.*[5-9]|Back-off restarting

## Oomkilled Silent Cascade

### **Id**
oomkilled-silent-cascade
### **Summary**
OOMKilled pod takes down other pods on same node via memory pressure
### **Severity**
critical
### **Situation**
One pod OOMKilled, then other pods on same node start failing
### **Why**
...
### **Detection Pattern**
OOMKilled|Exit Code.*137|MemoryPressure
...
imagepullbackoff-registry-auth
### **Summary**
ImagePullBackOff after working fine - registry auth token expired
### **Severity**
high
### **Situation**
Deployment worked yesterday, today ImagePullBackOff on new pods
### **Why**
...
### **Detection Pattern**
ImagePullBackOff|unauthorized.*pull|authentication required
...
service-selector-mismatch
### **Summary**
Service selector doesn't match pod labels - traffic goes nowhere
### **Severity**
high
### **Situation**
Service exists, endpoints empty, no traffic reaching pods
### **Why**
...
## Configmap Not Reloading

### **Id**
configmap-not-reloading
### **Summary**
ConfigMap changed but pods still using old config - no auto-reload
### **Severity**
high
### **Situation**
Updated ConfigMap, pods don't pick up changes, service misbehaving
### **Why**
...
### **Detection Pattern**
configMapRef|configmap.*mount

## Helm Release State Desync

### **Id**
helm-release-state-desync
### **Summary**
Helm thinks release succeeded but resources don't match - state desync
### **Severity**
high
### **Situation**
helm upgrade succeeded, but resources are wrong, rollback doesn't fix
### **Why**
...
### **Detection Pattern**
helm.*upgrade|helm.*rollback|release.*state

## Resource Quota Silent Rejection

### **Id**
resource-quota-silent-rejection
### **Summary**
ResourceQuota blocking pod creation but no obvious error
### **Severity**
medium
### **Situation**
Deployment scaled to 10, only 5 pods running, no error in deployment status
### **Why**
...
### **Detection Pattern**
ResourceQuota|exceeded quota|FailedCreate

## Pvc Wrong Access Mode

### **Id**
pvc-wrong-access-mode
### **Summary**
PVC access mode wrong for pod scheduling - pods stuck Pending
### **Severity**
medium
### **Situation**
Pod stuck in Pending, needs volume, PVC bound but pod can't mount
### **Why**
...
### **Detection Pattern**
ReadWriteOnce|accessModes|PersistentVolumeClaim
...
ingress-class-conflict
### **Summary**
Multiple ingress controllers - wrong one handles your Ingress
### **Severity**
medium
### **Situation**
Ingress created but not working, logs show different controller
### **Why**
...
### **Detection Pattern**
ingressClassName|ingress.*controller|Ingress.*class

## Init Container Blocking Forever

### **Id**
init-container-blocking-forever
### **Summary**
Init container stuck - main containers never start
### **Severity**
medium
### **Situation**
Pod shows Init:0/2, main app containers never run
### **Why**
...
### **Detection Pattern**
initContainers|Init:|PodInitializing
...
liveness-probe-too-aggressive
### **Summary**
Liveness probe kills healthy pods under load
### **Severity**
high
### **Situation**
Pods getting killed during traffic spikes, app was healthy
### **Why**
...
### **Detection Pattern**
livenessProbe|failureThreshold.*1|timeoutSeconds.*1