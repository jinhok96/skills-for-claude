...
## Pid1 Signal Handling

### **Id**
pid1-signal-handling
### **Summary**
SIGTERM not reaching application - container takes 30 seconds to stop
### **Severity**
critical
### **Situation**
Container stop takes exactly 30 seconds (Docker's default timeout), then SIGKILL
### **Why**
...
### **Detection Pattern**
CMD\s+[a-zA-Z]|ENTRYPOINT\s+[a-zA-Z](?!\[)

## Apt Get Cache Invalidation

### **Id**
apt-get-cache-invalidation
### **Summary**
apt-get update cached but packages fail to install
### **Severity**
high
### **Situation**
Docker build fails with "Unable to locate package" even though package exists
### **Why**
...
### **Detection Pattern**
apt-get update\s*$|apt-get update\s*\n\s*RUN
...
secrets-baked-in-image
### **Summary**
Secrets accidentally included in image layers, visible to anyone who pulls
### **Severity**
critical
### **Situation**
API keys, passwords, or credentials accessible via docker history or layer extraction
### **Why**
...
### **Detection Pattern**
ARG.*KEY|ARG.*TOKEN|ARG.*PASSWORD|ARG.*SECRET|COPY.*\.env
...
layer-cache-invalidation
### **Summary**
Minor code change rebuilds all layers - 20 minute builds
### **Severity**
high
### **Situation**
Every build reinstalls dependencies even though they haven't changed
### **Why**
...
alpine-musl-compatibility
### **Summary**
Application crashes or behaves differently on Alpine due to musl libc
### **Severity**
high
### **Situation**
Works on Ubuntu/Debian, crashes or hangs on Alpine
### **Why**
...
## Build Context Explosion

### **Id**
build-context-explosion
### **Summary**
Build context is gigabytes - builds take forever to start
### **Severity**
medium
### **Situation**
"Sending build context to Docker daemon" takes minutes, uploading gigabytes
### **Why**
...
zombie-processes
### **Summary**
Container accumulates zombie processes, eventually hits PID limit
### **Severity**
medium
### **Situation**
Container runs for days, then starts failing with "fork: Resource temporarily unavailable"
### **Why**
...
### **Detection Pattern**
CMD\s+\[|ENTRYPOINT\s+\[(?!.*tini)(?!.*dumb-init)
...
non-reproducible-builds
### **Summary**
Same Dockerfile produces different images on different machines
### **Severity**
medium
### **Situation**
Build works locally, fails in CI, or vice versa
### **Why**
...
healthcheck-ignored
### **Summary**
Container reports healthy but application is broken
### **Severity**
medium
### **Situation**
Load balancer sends traffic to unhealthy container because HEALTHCHECK passed
### **Why**
...
### **Detection Pattern**
HEALTHCHECK.*curl|HEALTHCHECK.*wget
...
large-image-size
### **Summary**
Image is 2GB when it should be 200MB - slow deploys, wasted storage
### **Severity**
medium
### **Situation**
Image pull takes 5+ minutes, registry storage explodes, EKS bills spike
### **Why**
...
### **Detection Pattern**
FROM.*:latest|FROM ubuntu|FROM debian(?!.*slim)